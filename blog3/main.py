...
async def get_records(
    filename: str = Query(..., description="The filename to retrieve")
):
    try:
...
async def create_upload_file(
    file: UploadFile = File(...), filepath: str = "/path/to/files/"
):
    # Save file information to the database
    with database.atomic():
        uploaded_file = Files_Uploader.create(
            filename=file.filename, filepath=filepath + file.filename
        )
